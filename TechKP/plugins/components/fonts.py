...
from TechKP.plugins.components.fotnt_string import Fonts
import os
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup


@Client.on_message(filters.private & filters.command(["font"]))
async def stylezg_buttons(c, m, cb=False):
    buttons = [[
        ...
        InlineKeyboardButton('Next ➡️', callback_data="cdnxt")
    ]]
    if not cb:
        if ' ' in m.text:
...
@Client.on_callback_query(filters.regex('^cdnxt'))
async def dcnxt(c, m):
    if m.data == "cdnxt":
        buttons = [[
